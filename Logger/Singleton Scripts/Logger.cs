...
#region ==============================|       Public Constansts      |==============================
    ...
    public const string UNITY_TAG = "UnityTag";
    public const string EXTRA_TAGS = "ExtraTags";
    public const string TAG = "Tag";
    public const string STATE = "State";
    public const string END_STATE = "End_State";
    public const string START_STATE = "Start_State";
...
#region ==============================|        Private Methods       |==============================
	...
	private string FormatObject(GameObject gob) {
		return System.String.Format("<{3}><{4}>{0}</{4}>{1}{2}</{3}>",
		                                gob.name, FormatLoggableObject(gob), FormatTransform(gob),OBJECT,NAME);
	}
	
	private string FormatLoggableObject(GameObject gob) {
		LoggableObject lob = gob.GetComponent<LoggableObject>();
		if(lob == null)
			return string.Empty;
		else 
			return lob.LogDescription();
	}
	...
	private string FormatTransform(GameObject gob) {
		if (gob.transform.GetComponent<Rigidbody>() == null) {
			...
			return System.String.Format("<{3}><{4}><X>{0}</X><Y>{1}</Y></{4}><{4}>{2}</{4}></{3}>",
			...
			return System.String.Format("<{6}><{7}><X>{0}</X><Y>{1}</Y></{7}><{8}>{2}</{8}><{9}><X>{3}</X><Y>{4}</Y></{9}><{10}>{5}</{10}></{6}>",
		                                gob.transform.position.x,
		                                gob.transform.position.y,
		                                gob.transform.rotation.z,
...
#region GameObject Events
	...
	public void ObjectAction(GameObject gob, string action) {
		...
		this.write(gob.name,action,this.FormatObject(gob));
	}
	...
	public void ObjectAction(GameObject gob, string action, bool withState) {
		...
		this.write(gob.name, action, this.FormatObject(gob),guid);
	}
		
    #endregion GameObject Events
...
#region Session Events
	...
	private List<GameObject> StartStateObjects = new List<GameObject>();
	
	public void AddStartStateObject(GameObject gob) {
		if (!this.Enabled) return;
		
		if (this.StartStateObjects != null && !this.StartStateObjects.Contains(gob)) {
			this.StartStateObjects.Add(gob);
		}
	}
	
	public void RemoveStartStateObject(GameObject gob) {
		if (!this.Enabled) return;
		
		if (this.StartStateObjects != null && this.StartStateObjects.Contains(gob)) {
			this.StartStateObjects.Remove(gob);
		}
	}
	
	public void ClearStartStateObjects() {
		if(!this.Enabled) return;
		
		this.StartStateObjects.Clear();
	}
	
	public void RecordStartState() {
		this.RecordStartState(StartStateObjects);
	}
